...
export default function ProfileActivities() {
    ...
    /* useEffect impact:-
        Calling setFilter, set the value of first tab as 'future'
        ,then this updates also update the state of setFilter inside your useProfile hook, 
        which make this conditon(enabled: !!id && !!filter) true in the useProfile hook, 
        triggering the query to fetch user activities(api call)
        which causes this component to re-render.
    */
    useEffect(() => {           // Starts a React effect that will run after render and can re-run on dependency changes.
        setFilter('future')     // Sets the filter state to show future events by default.
    }, [setFilter])             // Limits the effect to run on mount and when the setFilter function reference changes.
